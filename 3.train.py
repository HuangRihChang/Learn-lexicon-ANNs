...
def save(folder, params, epoch):   
    for param in params:
        current_name = epoch + param.name +'_current.npy'
        last_name = epoch+ param.name +'_last.npy'
        if os.path.exists(os.path.join(folder, current_name)):
            if os.path.exists(os.path.join(folder, last_name)):
                remove(os.path.join(folder,last_name))
            rename(os.path.join(folder,current_name), os.path.join(folder,last_name))
...
def train_sowe(folder,
    ...
    while (epoch<n_epochs): 
        ...
        for minibatch in grouper:
            ...
            idx+=1
            print ('[learning] epoch %i >> %2.2f%%'%(epoch,total_examples*100./num_trains) + ' completed in %.2f (sec)'%(time.time()-tic), end=' \r')
...
parser.add_argument('languge', type=str, help='input en, vi, zh, ar')
...
#INPUT FILES
data_lang = lang+"-data"
lexicon_file = "mylexicon-"+lang

traininfofile= os.path.join("./train_data",data_lang,"info.tw")
trainfile=os.path.join("./train_data",data_lang,"process.tw")
#OUTPUT FILE
mylexiconfolder = os.path.join("./data","lexicons",lexicon_file)

dst= os.path.join("./data","lexicons",lexicon_file,"info.tw")

#create folder if not exist:
if not os.path.exists("./data"): os.mkdir("./data")
if not os.path.exists("./data/lexicons"): os.mkdir("./data/lexicons")
if not os.path.exists("./data/lexicons/"+lexicon_file): os.mkdir("./data/lexicons/"+lexicon_file)

#copy file info.tw
copyfile(traininfofile, dst)
...
tic = time.time()
perf = train_sowe( mylexiconfolder,
                  trainsentences,
                   word_idx_map,
                   max_l,
                   num_trains,
                   ...
                   n_epochs=5, 
                   batch_size=50, 
                   lr_decay = 0.95, #0.1 0.5 1 (best)0.95 0.97
                   sqr_norm_lim=9,
                   non_static=True,
                   report=100)
print (perf)
